<template>
    <div class="container">
        <div class="card mt-2">
<!--            <h4 class="card-header text-info">{{ cardTitle }}-->
<!--                <button v-if="showTime" type="button" class="btn btn-danger animated pulse infinite">-->
<!--                    Exam End Time: <span class="badge badge-light">{{ endTime }}</span>-->
<!--                </button>-->
<!--            </h4>-->
                        <h4 class="card-header text-info">{{cardTitle}} <span
                            class="float-right text-danger animated flash delay-3s">Paper time:1 Hour <small> remaining time in Mins is given below</small></span><button v-if="showTime" type="button" class="btn btn-danger animated pulse infinite">
                            Exam End Time: <span class="badge badge-light">{{endTime}}</span>
                        </button></h4>
            <div class="card-body overflow-auto">
                <v-btn fab dark x-large fixed right bottom>
                    <v-icon dark>{{ remainingTimeMins }}</v-icon>
                </v-btn>

                <ul :id="m.paperid" class="list-group mt-2" v-for="(m,i) in mcqsData.data">
                    <li class="list-group-item active py-3">{{ ++i }}: <span
                        class="ml-3 blockquote">{{ m.question }}</span>
                        <span
                            class="badge badge-light float-right">attempted:{{ AttemptedMcqs }} total:{{ McqsCount }}</span>
                    ...
                    <li v-if="m.fourth" :key="m.paperid+'4'" class="list-group-item py-2">D:<span
                        class="ml-4 blockquote">{{ m.fourth }}</span><span :id="m.paperid+'4'"
                                                                           class="float-right"><input
            ...
            endPaperCheck() {
                axios.get('api/result/user_course')
                    .then(response => {
                        ...
                        let endtime = new moment(etime, 'HH:mm:ss').subtract(10, 'minutes').format('HH:mm:ss');
                        ...
                        setInterval(() => {
                            ...
                            // var mins = endtime.diff(current_time,'mm');
                            this.remainingTimeMins = moment.utc(moment(etime, "HH:mm:ss").diff(moment(current_time, "HH:mm:ss"))).format("mm");
                            this.remainingTimeMins= parseInt(this.remainingTimeMins)+1;
