<template>
    <div class="container">
        <h3 class="text-danger" v-if="courseData.length>0"><span class="badge badge-primary text-white">Previous Trainings Data</span></h3>
        ...
        <div class="d-none" id="trainees_data">
            <h3 class="text-danger"><span class="badge badge-danger text-white">Currently Enrolled Training</span></h3>
            <table class="table table-dark">
                ...
                <tr>
                    ...
                    <td><button :class="examClass" title="For Paper Start Click Here.." @click="startExam()">Start Paper</button></td>
                </tr>
                </tbody>
            </table>
        </div>
        <div id="sch_exam">
            <h3 class="text-danger"><span class="badge badge-danger text-white">Scheduled Exams</span></h3>
            <table class="table table-dark">
                <thead>
                <tr>
                    <th>Course</th>
                ...
                <tr v-for="e in examData">
                    ...
                    <td>{{e.etime}}</td>
                    <td></td>
                    <td><button class="btn btn-sm btn-info text-white" v-if="e.exam_status==1" title="For Paper Start Click Here.." @click="startExam(e.exam_id)">Start Paper</button></td>
                </tr>
                </tbody>
            </table>
        </div>
        <div class="d-none" id="ins_data">
            <h3 class="text-danger"><span class="badge badge-danger text-white">Individual Schedule</span></h3>
            <table class="table table-dark">
                <thead>
                <tr>
                    <th>#</th>
                ...
                <tr :key="index" v-for="(sch,index) in schData">
                    ...
                    <td>{{ sch.name }}</td>
                    <th>
                        <a :href="'../indschedual/'+sch.file" target="_blank" :title="sch.file"><i class="fas fa-2x fa-file-download"></i></a>
                    </th>
        ...
        <div class="card table-responsive" v-if="tableLength">
            ...
            <table class="table">
                ...
                <tr :key="index" v-for="(data,index) in files">
                    ...
                    <td>{{data.filetype}}</td>
                    <td><a :href="'../indschedual/'+data.file" target="_blank" :title="data.file"><i class="fas fa-2x fa-file-download"></i></a></td>
                </tr>
...
<script>
    ...
    export default {
        ...
        methods: {
            getAnnouncements()
            {
                axios.get(this.baseUrl+'api/display_data/onLogannouncement-')
                    .then((response) => {
            ...
            getExamData()
            {
                axios.get(this.baseUrl+'api/exam/tEmail-'+this.traineeEmail)
                    .then(response => {
            ...
            getSchedules() {
                axios.get(this.baseUrl+'api/display_data/instructor-'+this.traineeEmail)
                    .then(response => {
            ...
            userTableTrainee() {
                axios.get('api/display_data/content')
                    ...
                    .catch((error) => console.log("error files/content occured in files retrieval.."));
            },
            startExam(exam)
            {
                axios.get(this.baseUrl+'api/exam/'+exam)
                    .then((response)=>{
                        ...
                        {
                            swal('Notice!','You may have already submitted result or Kindly wait till the starting time..Starting Time: '+stime,'warning');
                        }
                    })
                    .catch(error=>console.log(error));
            },
            checkDataFunc()
            {
              axios.get(this.baseUrl+'api/display_data/dataCheck')
                .then(response=>{
            ...
            getTraineeData() {
                axios.get(this.baseUrl+'api/display_data/onLogin')
                    .then(response => {
                        if (response.data.length > 0) {
            ...
            getResults() {
                ...
                axios.get(this.baseUrl+'api/display_data')
                    .then(response => {
            ...
            checkExam()
            {
                axios.get(this.baseUrl+'api/exam/trainee_exam')
                    .then(response=>{
